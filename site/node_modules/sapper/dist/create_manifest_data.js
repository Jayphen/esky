...
function generate_server_manifest(
	...
	const imports = [].concat(
		manifest_data.server_routes.map((route, i) =>
			`import * as route_${i} from ${index.stringify(index.posixify(`${path_to_routes}/${route.file}`))};`),
		manifest_data.components.map((component, i) =>
			`import * as component_${i} from ${index.stringify(get_file(path_to_routes, component))};`),
		`import * as root_comp from ${index.stringify(get_file(path_to_routes, manifest_data.root))};`,
		`import error from ${index.stringify(get_file(path_to_routes, manifest_data.error))};`
	);
...
var transitiveDeps = /*@__PURE__*/unwrapExports(dist);
...
function extract_css(
 ...
 = {
	...
	components.forEach(component => {
		const resolved = index.normalize_path(path.resolve(dirs.routes, component.file));
...
class RollupResult  {
	...
	constructor(duration, compiler, sourcemap) {
		...
		this.summary = compiler.chunks.map(chunk => {
			const size_color = chunk.code.length > 150000 ? index.kleur.bold().red : chunk.code.length > 50000 ? index.kleur.bold().yellow : index.kleur.bold().white;
			...
			deps.forEach((dep, i) => {
				...
				lines.push(index.kleur.gray(line));
			});
...
function handleError(err, recover = false) {
	...
	stderr(index.kleur.bold().red(`[!] ${index.kleur.bold(message.toString())}`));

	if (err.url) {
		stderr(index.kleur.cyan(err.url));
	}
	...
	if (err.frame) {
		stderr(index.kleur.dim(err.frame));
	}

	if (err.stack) {
		stderr(index.kleur.dim(err.stack));
	}
...
function formatMessage(message, isError) {
  ...
  lines[0] = index.kleur.inverse(lines[0]);
...
function get_slug(file) {
	...
	if (index.reserved_words.has(name)) name += '_';
	return name;
}
