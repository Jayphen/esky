...
class Watcher extends EventEmitter.EventEmitter {
	...
	async init() {
		if (this.port) {
			if (!await Deferred.check(this.port)) {
				this.emit('fatal', {
			...
			this.port = await Deferred.find(3000);
		}
		...
		if (this.bundler === 'rollup') copy_runtime.copy_shimport(dest);

		if (!this.dev_port) this.dev_port = await Deferred.find(10000);

		// Chrome looks for debugging targets on ports 9222 and 9229 by default
		if (!this.devtools_port) this.devtools_port = await Deferred.find(9222);
		...
		const compilers = await create_manifest_data.create_compilers(this.bundler, cwd, src, dest, true);
