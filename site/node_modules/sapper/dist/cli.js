...
if (process.argv[2] === 'start') {
	...
	console.error(index.kleur.bold().red(`'sapper start' has been removed`));
...
prog.command('dev')
	...
	.action(async (opts
		...
		const { dev } = await Promise.resolve().then(function () { return require('./dev.js'); });
		...
		try {
			...
			watcher.on('ready', async (event) => {
				if (first) {
					console.log(index.kleur.bold().cyan(`> Listening on http://localhost:${event.port}`));
					if (opts.open) {
						const { exec } = await Promise.resolve().then(function () { return _interopNamespace(require('child_process')); });
			...
			watcher.on('invalid', (event) => {
				...
				console.log(`\n${index.kleur.bold().cyan(changed)} changed. rebuilding...`);
			});

			watcher.on('error', (event) => {
				const { type, error } = event;

				console.log(index.kleur.bold().red(`✗ ${type}`));

				if (error.loc && error.loc.file) {
					console.log(index.kleur.bold(`${path.relative(process.cwd(), error.loc.file)} (${error.loc.line}:${error.loc.column})`));
				}

				console.log(index.kleur.red(event.error.message));
				if (error.frame) console.log(error.frame);
			});

			watcher.on('fatal', (event) => {
				console.log(index.kleur.bold().red(`> ${event.message}`));
				if (event.log) console.log(event.log);
			});

			watcher.on('build', (event) => {
				if (event.errors.length) {
					console.log(index.kleur.bold().red(`✗ ${event.type}`));

					event.errors.filter(e => !e.duplicate).forEach(error => {
						if (error.file) console.log(index.kleur.bold(error.file));
					...
					console.log(index.kleur.bold().yellow(`• ${event.type}`));

					event.warnings.filter(e => !e.duplicate).forEach(warning => {
						if (warning.file) console.log(index.kleur.bold(warning.file));
					...
					console.log(`${index.kleur.bold().green(`✔ ${event.type}`)} ${index.kleur.gray(`(${index.format_milliseconds(event.duration)})`)}`);
				}
			});
		} catch (err) {
			console.log(index.kleur.bold().red(`> ${err.message}`));
			console.log(index.kleur.gray(err.stack));
			process.exit(1);
		}
	});
...
prog.command('build [dest]')
	...
	.action(async (dest = '__sapper__/build', opts
		...
		try {
			...
			console.error(`\n> Finished in ${index.elapsed(start)}. Type ${index.kleur.bold().cyan(`node ${dest}`)} to run the app.`);
		} catch (err) {
			console.log(`${index.kleur.bold().red(`> ${err.message}`)}`);
			console.log(index.kleur.gray(err.stack));
			process.exit(1);
	...
	.action(async (dest = '__sapper__/export', opts
		...
		try {
			if (opts.build) {
				...
				console.error(`\n> Built in ${index.elapsed(start)}`);
			}

			const { export: _export } = await Promise.resolve().then(function () { return require('./export.js'); });
			const { default: pb } = await Promise.resolve().then(function () { return require('./index2.js'); }).then(function (n) { return n.index; });
			...
			await _export({
				...
				oninfo: event => {
					console.log(index.kleur.bold().cyan(`> ${event.message}`));
				},

				onfile: event => {
					const size_color = event.size > 150000 ? index.kleur.bold().red : event.size > 50000 ? index.kleur.bold().yellow : index.kleur.bold().gray;
						const size_label = size_color(index.left_pad(pb(event.size), 10));

						const file_label = event.status === 200
							? event.file
							: index.kleur.bold()[event.status >= 400 ? 'red' : 'yellow'](`(${event.status}) ${event.file}`);

						console.log(`${size_label}   ${file_label}`);
				}
			});

			console.error(`\n> Finished in ${index.elapsed(start)}. Type ${index.kleur.bold().cyan(`npx serve ${dest}`)} to run the app.`);
		} catch (err) {
			console.error(index.kleur.bold().red(`> ${err.message}`));
			process.exit(1);
		}
	});
...
async function _build(
	...
	const { build } = await Promise.resolve().then(function () { return require('./build.js'); });
	...
	await build({
		...
		oncompile: event => {
			...
			let c = (txt) => index.kleur.cyan(txt);

			const { warnings } = event.result;
			if (warnings.length > 0) {
				banner += ` with ${warnings.length} ${warnings.length === 1 ? 'warning' : 'warnings'}`;
				c = (txt) => index.kleur.cyan(txt);
			}

			console.log();
			console.log(c(`┌─${index.repeat('─', banner.length)}─┐`));
			console.log(c(`│ ${index.kleur.bold(banner)       } │`));
			console.log(c(`└─${index.repeat('─', banner.length)}─┘`));
